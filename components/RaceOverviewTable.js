...
export default (props) => {
    const { race, wide, verboseOdds, configuringPrediction, predictionEdited } = props;
        ...
        <table className="table">
            <thead className="table-heading">
                <tr>
                    ...
                    { configuringPrediction && <th className="padding-left-1">
                        <span>Amount </span>
                        <button type="button" className="link-button">(?)</button>
                    </th>}
                </tr>
                ...
                return (
                    <tr key={++key} style={{ color }}>
                        <td>
                            ...
                            { candidate.incumbent && <span className="incumbent" style={{ backgroundColor: color }}>i</span> }
                        </td>
                        <td>{odds}</td>
                        { configuringPrediction && <th>
                            <input 
                                type="text" 
                                className="table-text-input" 
                                value={predictions[i] ? predictions[i] : ''}
                                placeholder="0"
                                onChange={(e) => onPredictionEditedInternal({ candidate, index: i, event: e })}
                            />
                        </th>
                        }
                    </tr>
